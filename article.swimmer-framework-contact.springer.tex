...
\titlerunning{A computational framework for swimmers}
...
All the implementations are made using the \feelpp\ finite element library \cite{prudhomme_feelppfeelpp_2023}.}
...
In this case, the hydrodynamic drag can be simplified asymptotically by employing the Resistive Force Theory \cite{gray_propulsion_1955}, in which the hydrodynamic friction is related to the fluid velocity via an %prescribed 
...
%\review{However, this approach is computationally costly.}
%the CUTFEM approach in which mesh intersections are computationally costly.
...
This approach is popular in fluid-structure interaction problems \cite{chabannes_high-order_2013,pena_high_2012}, and it is based on a formulation of the fluid equations in an intermediate frame between the Eulerian and Lagrangian description.
...
%\review{Furthermore, when comparing this method to the CUTFEM method, in one instance, the high computational cost arises from projecting the swimmer onto the existing mesh, whereas in our case, the cost is associated with remeshing.}
...
\end{tikzpicture}
\end{center}
   \caption{Notations for the fluid-swimmer model. $\CompDomain_t$ denotes the fluid domain and $\Solid^i_t,\Solid^j_t,\Solid^k_t$ denote the swimmers and solid objects included in the fluid. $\partial \Fluid_{t,N}$ and $\partial \Fluid_{t,D}$ denote the portions of the fluid boundary where Dirichlet and Neumann boundary conditions are prescribed.}
...
%while the second one is given by the deformation velocity of the solid $\Vel_d^i$, if present. 
...
%, or external forces such as gravity or collision forces. 

Let us define the mass of the $i$-th swimmer by  $\mass^i = \int_{\Solid^i} \Density_{\Solid}^i$, where $\Solid^i = \Solid^i_0$ is the reference configuration, %, let $\CenterMassi = \frac{1}{\mass^i} \int_{\Solid^i} \Density_{\Solid}^i x$ be its center of mass, 
...
%for which a collision zone of width $\rho$ is defined in which the forces are activated.
...
%decouples the evolution of the computational domain  %from the evolution of the fluid. 
...
%\begin{multline}
%	\bar{\phi}^{t_{n+1}}(X) =  R(\theta_{n+1}) (X+\bar{\phi}_1^{t_{n+1}}(X)- X_{CM} - \phi_1^{t_{n+1}}(X_{CM})) 
 %+ \phi_1^{t_{n+1}}(X_{CM}^0)-X.
%	\label{Eq:discDisp2}
%\end{multline}
\begin{multline}
...
%The test functions satisfy the velocity constraint on the boundary of each swimmer. Thus, one has: 
...
%	\int_{\CompDomain_{t_{n+1}}} \rho_f\partial_t \Vel_h^{n+1}|_\Alemap  \cdot \tilde{\Vel} + \int_{\CompDomain_{t_{n+1}}} \rho_f((\Vel_h^{n+1}-{\Vel_{\mathcal{A}}}_h^{n+1} )\cdot \nabla u_h^{n+1}) \cdot \tilde{\Vel}\\
 %+ 2\mu \int_{\CompDomain_{t_{n+1}}} D(\Vel_h^{n+1}) : D( \tilde{\Vel}) - \int_{\CompDomain_{t_{n+1}}} \Pres_h^{n+1} \nabla \cdot \tilde{\Vel} - \int_{} \sigma(u,p)\cdot n \cdot \tilde{\Vel}\\
% = \int_{\CompDomain_{t_{n+1}}} f^{n+1} \cdot \tilde{\Vel}\\
% = \int_{\CompDomain_{t_{n+1}}} f^{n+1} \cdot \tilde{\Vel} 
...
%	-\int_{\CompDomain_{t_{n+1}}} \Pres_h^{n+1} \nabla \cdot \tilde{\Vel} = \int_{\CompDomain_{t_{n+1}}} f^{n+1} \cdot \tilde{\Vel},\\
%\sum_{i=1}^N m^i\frac{d\tvel^i}{dt}^{n+1}\cdot \tilde{\tvel}^i = -\int_{\partial \Solid^i} \sigma \normal \tilde{\tvel}^i + F_e^i \tilde{\tvel}^i
%\\

 %\sum_{i=1}^N \frac{d[R^i(t)I^iR^i(t)^T\angvel^i]}{dt}^{n+1} \cdot \tilde{\angvel}^i =  -\int_{\partial \Solid^i} \sigma \times (x-\CenterMassi) \tilde{\angvel}^i + T_e^i \tilde{\angvel}^i 
	%\label{Eq:discrete-var_form}
%\end{eqnarray}
...
%	\int_{\CompDomain_{t_{n+1}}} \rho_f\partial_t \Vel_h^{n+1}|_\Alemap  \cdot \tilde{\Vel} + \int_{\CompDomain_{t_{n+1}}} \rho_f((\Vel_h^{n+1}-{\Vel_{\mathcal{A}}}_h^{n+1} )\cdot \nabla  u_h^{n+1}) \cdot \tilde{\Vel}+
 %\\
 %+ \sum_{i=1}^N \frac{d[R^i(t)I^iR^i(t)^T\angvel^i]}{dt}^{n+1} \cdot \tilde{\angvel}^i+ \sum_{i=1}^N m^i\frac{d\tvel^i}{dt}^{n+1}\cdot \tilde{\tvel}^i +
%	\\ + 
%	2\mu\int_{\CompDomain_{t_{n+1}}} [(\nabla \Vel_h^{n+1} + \nabla \Vel_h^{n+1,T})] :  [(\nabla \tilde{\Vel} + \nabla \tilde{\Vel}^T)]  \, dx  %-\int_{\partial \Solid^h} (-\Pres_h^{n+1} \mathbb{I} + 2\mu D(\Vel_h^{n+1})) \normal \cdot v\, dS
%	-\int_{\CompDomain_{t_{n+1}}} \Pres_h^{n+1} \nabla \cdot \tilde{\Vel} \\
 %= \int_{\CompDomain_{t_{n+1}}} f^{n+1} \cdot \tilde{\Vel} + \sum_{i=1}^N F_e^i \cdot \tilde{\tvel}^i + \sum_{i=1}^N T_e^i \cdot \tilde{\angvel}^i,
	%\label{Eq:discrete-var_form}
%\end{multline}
...
%The boundary terms $	\int_{\partial \Solid} \sigma \normal \cdot\tilde{\Vel} $ are never computed in the assembly of the system matrix: 
%instead of building the finite element basis $(\tilde{\Vel},\tilde{\Pres},\tilde{\tvel},\tilde{\angvel}) \in [V_{h}^{t_{n+1}} \times  Q_h^{t_{n+1}} \times \mathbb{R}^d \times \mathbb{R}^{d^*}]$ that satisfies boundary conditions $\Vel = \tvel+ \angvel \times (x-x_{CM}) + u_d$ on  $\partial \Solid$, 
\color{black}
We now discretize the previous equations as %\eqref{Eq:discrete-var_form}-\eqref{Eq:discrete-var_form2} 
%using the unconstrained basis, getting the following saddle point problem
...
Finally, plugging \eqref{eq:boundary_constraint} into \eqref{Eq:flu-rig-block}, we then obtain the discretization of the variational formulation of the fluid-swimmer problem %\eqref{Eq:discrete-var_form}-\eqref{Eq:discrete-var_form2}
\begin{equation*}
...
\end{equation*}.
\color{black}
%We solve the previous system using the saddle point framework.
...
Using the two distance fields $D_i^{NB}$ and $D_j^{NB}$, the distance $d_{ij}$ is described by:
$$
...
We present numerical simulations of swimming micro-organisms to illustrate the framework we have presented in this chapter. The results are obtained using the \feelpp\ finite element library \cite{prudhomme_feelppfeelpp_2023}.
...
In this application, remeshing %and interpolation 
...
%In this paper, we provide an framework using finite element methods with ALE approach for simulate the dynamics of rigid deformable swimmers. Moreover, These simulations take into account the effect of collisions for both swimmer-swimmer interactions and interaction between swimmers with the wall. Finally, our approach allows to compute the dynamics of the swimmers into complex geometrical environments. All this are implemented using the finite elements library Feel++ \cite{feelpp}.
\color{black}
...
% demander à Christophe si on peut citer une toolbox accessible ici ??
